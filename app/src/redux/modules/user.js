...
export const GET_USER_GAMES = `${typeBase}GET_USER_GAMES`;
export const ADD_GAME_TO_USER = `${typeBase}ADD_GAME_TO_USER`;
...
export const getUserGames = () => async (dispatch) => {
  try {
    const response = await get(dispatch, GET_USER_GAMES, USER_ENDPOINT_BASE, true);
    return Promise.resolve(response);
  } catch (err) {
    await handleError(dispatch, err, GET_USER_GAMES);
  }
};

export const addGameToUser = (gameId) => async (dispatch) => {
  try {
    const response = await post(dispatch, ADD_GAME_TO_USER, USER_ENDPOINT_BASE, gameId, true);
    return Promise.resolve(response);
  } catch (err) {
    await handleError(dispatch, err, ADD_GAME_TO_USER);
  }
}
